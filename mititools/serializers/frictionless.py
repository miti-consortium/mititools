...
def write_frictionless(top_variables, data_tables, cv_tables):
    ...
    with open(join(fd_package_path, json_filename), 'wt') as f:
        f.write(payload)
    if not exists(fd_schema_path):
        mkdir(fd_schema_path)
    with open(join(fd_schema_path, json_filename), 'wt') as f:
        f.write(payload)
